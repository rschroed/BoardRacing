...
| Resume countdown | The shared `3`, `2`, `1` sequence, exactly like the race start | Get ready; the race resumes where it paused | Lifting an unfinished racer's Ship falls back to the pause, never to a fresh grid; no race state changes during the countdown |
...
| Final results | Large `RACE FINISHED` overlay: winner line plus a `START NEW RACE` touch button (owner decision, 2026-07-19, issue #97) | Touch START NEW RACE, or move both Ships to Brake and hold for the piece rematch | The overlay owns the center after the race; racing guidance is removed; the Brake-hold rematch gesture is unchanged |
...
| Table-clear pause | every unfinished racer's Ship off the table for `0.75 s` mid-race pauses; just enough debounce that hands over the sensors don't trigger it (2 s read as lag on hardware) |
| Start new race (paused or finished) | `START NEW RACE` touch button on the pause and race-finished overlays — the game's one non-piece control, offered only when no race is running; resume countdown reuses the start countdown (`3 s`) |
| Course | the Wedge (`TrackCatalog`, issue #88): `6` laps; corner safe speeds sweeper `1.35×` · medium `1×` · tight `0.68×` of the tuned `190`; pit rejoin `850 px` along the `911 px` start/finish straight |
...
**Clearing the table pauses the race** (owner decision, 2026-07-19, issue #90): every unfinished racer's Ship off the table for `0.75 s` mid-race freezes the simulation exactly — positions, race clock, fuel, tires, and pit timers. The center overlay names both ways out. Replacing the unfinished racers' Ships starts a restart countdown identical to the race start and resumes the race where it stopped; lifting a Ship during that countdown returns to the pause, never to a fresh grid. Alternatively the overlay's **START NEW RACE touch button** resets to the starting grid exactly like a rematch. The same button appears on a large **`RACE FINISHED` overlay** when a race ends (owner decision, 2026-07-19, issue #97) — the invisible Brake-hold gesture alone failed the discoverability criterion in live play. The button is deliberately the game's **only non-piece control** (owner decision), offered only when no race is running — paused or finished — states where the table center is guaranteed clear and a screen touch cannot be confused with piece play. The post-finish Ships-to-Brake rematch gesture is unchanged and remains available as the piece path.

**The course is designed geometry** (owner decision, 2026-07-19, issue #88, option C): the placeholder octagon gave way to the **Wedge**, an asymmetric triangle authored directly in 1920×1080 screen space — the old `0.33` vertical presentation squash is gone, so simulation distance and drawn distance agree exactly. One lap reads as three corner characters: a fast right-side sweeper (R150, ~179° — on this wide shallow table the right corner is inherently a turnaround), a medium bottom-left corner (R84, ~62°), and a tight top-left hairpin (R78, ~119°) braking into the start/finish straight. Corner arcs and connecting straights are mutually tangent, and the simulation charges scrub and tire wear once per corner arc — never per drawn chord. Races run `6` laps to keep duration near the placeholder's five longer ones. More courses are planned (owner direction); the Wedge is the first entry in `TrackCatalog`.
