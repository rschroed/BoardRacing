...
| Start new race (while paused) | `START NEW RACE` touch button on the pause overlay — the game's one non-piece control; resume countdown reuses the start countdown (`3 s`) |
...
**Clearing the table pauses the race** (owner decision, 2026-07-19, issue #90): every unfinished racer's Ship off the table for `0.75 s` mid-race freezes the simulation exactly — positions, race clock, fuel, tires, and pit timers. The center overlay names both ways out. Replacing the unfinished racers' Ships starts a restart countdown identical to the race start and resumes the race where it stopped; lifting a Ship during that countdown returns to the pause, never to a fresh grid. Alternatively the overlay's **START NEW RACE touch button** resets to the starting grid exactly like a rematch. The button is deliberately the game's **only non-piece control** (owner decision): a pause is the one state where the pieces are off the table, so a screen touch cannot be confused with piece play, and the cleared center guarantees nothing sits on the button. The post-finish rematch flow is unchanged.
